...
var urlStub='http://localhost:5000/';
...
//  fetch(urlStub.concat('debug?param=').concat(message)).then(function(response) {
//      response.text().then(function(text) {
//      console.log("debug response: ".concat(text));
//      });
//});
...
//lightPower  
    thisOutput=fetch(urlStub.concat('getdb?param=lightsPower')).then(function(response) {
//        thisStatus=response.status;
//  var thisDebugObject=document.getElementById("presetOne");
//    thisDebugObject.style.fontSize = "26px";
//    thisDebugObject.style.color="#FFFFFF";
//    thisDebugObject.innerHTML=thisStatus;
        response.text().then(function(text) {
//  debug (response.split(",")[0]);
   ...
   //current light colour
    thisOutput=fetch(urlStub.concat('getdb?param=lightsColour')).then(function(response) {
        response.text().then(function(text) {
            sendColour("setting",text,false,false);
        });
    });
  // presets
    thisOutput=fetch(urlStub.concat('getpresets')).then(function(response) {
        response.text().then(function(text) {
            allPresets=text.split("|");
...
//    thisDebugObject.style.fontSize = "26px";
//    thisDebugObject.style.color="#FFFFFF";
//    thisDebugObject.innerHTML="HELLO";
    databaseTimer = window.setTimeout(getDatabase,databaseTimeout);
}
...
function screenOn() {
        ...
        fetch(urlStub.concat('screenon')).then(function(response) { });
        screenIsOn=true;
...
function screenOff() {
  client = new HttpClient();
  fetch(urlStub.concat('screenoff')).then(function(response) { });
	screenIsOn=false;
...
function toggleLights() {
    ...
    client = new HttpClient();
    fetch(urlStub.concat("lightspower?param=").concat(lightsAreOn)).then(function(response) { });
}
...
//      console.log	("default - thisElement is ".concat(thisElement));
  ...
  client = new HttpClient();
  if (setLights==true) {
          fetch(urlStub.concat("setlights?red=").concat(thisRed).concat("&green=").concat(thisGreen).concat("&blue=").concat(thisBlue)).then(function(response) { });
          }
  if (DB==true) {
          fetch(urlStub.concat("setdb?param=lightsColour&red=").concat(thisRed).concat("&green=").concat(thisGreen).concat("&blue=").concat(thisBlue)).then(function(response) { });
        } 
}
...
function setPreset(thisObject,thisHex) {
    ...
    fetch(urlStub.concat("setdb?param=").concat(thisObject.id).concat("&red=").concat(thisRed).concat("&green=").concat(thisGreen).concat("&blue=").concat(thisBlue)).then(function(response) { });
}
